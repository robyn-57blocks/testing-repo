...
/*
		...
		if (pre.classList) {
			pre.classList.remove('hide-item');
		} else {
			pre.className = pre.className.replace(new RegExp('(^|\\b)' + 'show'.split(' ').join('|') + '(\\b|$)', 'gi'), ' ');
		}
        
        if (next.classList) {
			next.classList.remove('hide-item');
		} else {
			next.className = next.className.replace(new RegExp('(^|\\b)' + 'show'.split(' ').join('|') + '(\\b|$)', 'gi'), ' ');
		}
*/
    ...
    var prepareSlideItem = function(slider, container, list, count, index, width) {
        ...
        for (i = 0; i < list.length; i++) {
	        addClass(list[i], 'hide-initial');
			addClass(list[i], 'hide');
		}
		//Show first slide
        for (i = 0; i < list.length; i++) {
		    if (i == 0)  {
			    removeClass(list[i], 'hide-initial');
			    removeClass(list[i], 'hide');
			    addClass(list[i], 'show');
		    }
		}
    ...
    Slider.prototype.slide = function(direction, diffX) {
        ...
        for (listElem = 0; listElem < list.length; listElem++) {
		    if (listElem === 0)  {
			    removeClass(list[listElem], 'hide-initial');
			    removeClass(list[listElem], 'hide');
			    addClass(list[listElem], 'show');
		    } else {
			    removeClass(list[listElem], 'show');
			    addClass(list[listElem], 'hide');
		    }
		}
